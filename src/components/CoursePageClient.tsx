    ...
    <div className="min-h-screen bg-white text-slate-900 font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <main className="lg:col-span-8 space-y-10">
          <section>
            <h2 className="text-2xl font-bold text-[#0A2540] mb-3">Course overview</h2>
            <p className="text-slate-600 leading-relaxed">{content.overview}</p>
          </section>

          <section>
            <h2 className="text-2xl font-bold text-[#0A2540] mb-4">What you'll learn</h2>
          ...
          <section className="grid sm:grid-cols-2 gap-8">
            <div>
              <h3 className="text-lg font-bold text-[#0A2540] mb-3 flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-[#0B5198]" /> Prerequisites
              </h3>
              <ul className="space-y-2">
                {content.prerequisites.map((p, i) => (
                  <li key={i} className="text-sm text-slate-600 flex items-start gap-2">
                    <span className="text-[#0B5198] mt-1">•</span> {p}
                  </li>
                ))}
              </ul>
            </div>
            <div>
              <h3 className="text-lg font-bold text-[#0A2540] mb-3 flex items-center gap-2">
                <Users className="w-5 h-5 text-[#0B5198]" /> Who should attend
              </h3>
              <ul className="space-y-2">
                {content.audience.map((a, i) => (
                  <li key={i} className="text-sm text-slate-600 flex items-start gap-2">
                    <span className="text-[#0B5198] mt-1">•</span> {a}
                  </li>
                ))}
              </ul>
            </div>
          ...
          <section className="bg-sky-50/60 border border-sky-100 rounded-2xl p-6">
            <h3 className="text-lg font-bold text-[#0A2540] mb-3 flex items-center gap-2">
              <Award className="w-5 h-5 text-[#0B5198]" /> Certification
            </h3>
            <p className="text-sm text-slate-600">{content.certification}</p>
          </section>
        </main>
        ...
        <aside className="lg:col-span-4">
          <div className="lg:sticky lg:top-28 space-y-6">
                ...
                <div className="border-t border-slate-100 pt-5">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {content.formats.map((f) => (
                      <span key={f} className="text-xs font-medium text-[#0B5198] bg-sky-50 border border-sky-100 px-2.5 py-1 rounded-full">{f}</span>
                    ))}
                  </div>
                </div>
            ...
            <Link
              href="/#courses"
              className="flex items-center justify-center gap-2 text-sm font-semibold text-[#0B5198] hover:text-[#003B99] transition-colors"
            >
      ...
      {related.length > 0 && (
        <section className="border-t border-slate-100 bg-slate-50/60">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <h2 className="text-2xl font-bold text-[#0A2540] mb-6">Related {course.vendorName} courses</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {related.map((r) => (
                <Link
                  key={r.slug}
                  href={`/courses/${r.slug}`}
                  className="group bg-white rounded-xl border border-slate-200 p-5 hover:border-sky-200 hover:shadow-md transition-all flex flex-col"
                >
                  <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">{r.domain || r.vendorName}</span>
                  <h3 className="text-sm font-semibold text-slate-800 group-hover:text-[#0B5198] transition-colors mt-1.5 leading-snug flex-1">{r.title}</h3>
                  <div className="flex items-center justify-between mt-4 text-xs text-slate-500">
                    <span>{r.level || 'Training'}</span>
                    <span className="flex items-center gap-1 text-[#0B5198] font-semibold group-hover:gap-1.5 transition-all">View <ChevronRight className="w-3.5 h-3.5" /></span>
                  </div>
                </Link>
              ))}
            </div>

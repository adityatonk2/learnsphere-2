...
export const CoursePageClient: React.FC<CoursePageClientProps> = ({ course, content, related }) => {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-sans">
      <Header onOpenContact={(s) => openEnroll(s || t('enquirySubjects.courseEnquiry'))} activeSection={active} setActiveSection={setActive} />
      ...
      <section className="relative bg-gradient-to-br from-[#0A2540] via-[#0B3A6B] to-[#0B5198] text-white overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          {/* Breadcrumb */}
          <nav className="flex items-center gap-1.5 text-xs text-sky-200/80 mb-6 flex-wrap" aria-label={t('breadcrumb.label')}>
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            <div className="max-w-3xl">
              <span className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider bg-white/10 border border-white/15 px-3 py-1 rounded-full">
                ...
                {course.vendorName} {course.domain ? `· ${course.domain}` : t('training')}
              </span>
              <h1 className="text-3xl sm:text-4xl lg:text-[2.75rem] font-extrabold leading-tight mt-4">
                {course.title}
              </h1>
              {course.code && (
                <p className="text-sky-200 mt-2 text-sm font-mono">{t('examTrack')} <span className="font-semibold text-white">{course.code}</span></p>
              )}
            ...
            <div className="flex flex-col gap-3 shrink-0 w-full lg:w-auto">
              ...
              >
                <Calendar className="w-5 h-5" /> {t('enrollButton')}
              </button>
              <button
                onClick={() => openEnroll(t('enquirySubjects.advisorCall', { title: course.title }))}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <main className="lg:col-span-8 space-y-10">
          <section>
            <h2 className="text-2xl font-bold text-[#0A2540] dark:text-white mb-3">{t('overviewHeading')}</h2>
            <p className="text-slate-600 dark:text-slate-300 leading-relaxed">{content.overview}</p>
          </section>

          <section>
            <h2 className="text-2xl font-bold text-[#0A2540] dark:text-white mb-4">{t('whatYoullLearnHeading')}</h2>
          ...
          <section className="grid sm:grid-cols-2 gap-8">
            <div>
              <h3 className="text-lg font-bold text-[#0A2540] dark:text-white mb-3 flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-[#0B5198] dark:text-sky-400" /> {t('prerequisitesHeading')}
              </h3>
            ...
            <div>
              <h3 className="text-lg font-bold text-[#0A2540] dark:text-white mb-3 flex items-center gap-2">
                <Users className="w-5 h-5 text-[#0B5198] dark:text-sky-400" /> {t('whoShouldAttendHeading')}
              </h3>
              <ul className="space-y-2">
          ...
          <div className="lg:sticky lg:top-28 space-y-6">
            <div className="rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              ...
              <div className="p-6 space-y-5">
                <div>
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('programIncludesLabel')}</span>
                ...
                <div className="border-t border-slate-100 dark:border-slate-800 pt-5">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('deliveryFormatsLabel')}</span>
                ...
                >
                  <Sparkles className="w-4 h-4" /> {t('scheduleButton')}
                </button>
                <p className="text-[11px] text-slate-400 text-center">{t('advisorReplyNote')}</p>
              </div>
            </div>
      ...
      {related.length > 0 && (
        <section className="border-t border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-900/60">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <h2 className="text-2xl font-bold text-[#0A2540] dark:text-white mb-6">{t('relatedCoursesHeading', { vendor: course.vendorName })}</h2>
              ...
              {related.map((r) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-4 text-xs text-slate-500 dark:text-slate-400">
                    <span>{r.level || t('training')}</span>
                    <span className="flex items-center gap-1 text-[#0B5198] dark:text-sky-400 font-semibold group-hover:gap-1.5 transition-all">{t('viewLink')} <ChevronRight className="w-3.5 h-3.5" /></span>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <Footer onOpenContact={() => openEnroll(t('enquirySubjects.courseEnquiry'))} setActiveSection={setActive} />

      ...
      <div ref={containerRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {LEADERS.map((leader) => (
            <div
              key={leader.name}
              className="gsap-reveal bg-slate-50 rounded-2xl border border-slate-200 p-8 flex flex-col items-center text-center hover:shadow-lg transition-all duration-300"
            >
              ...
              <div className="w-28 h-28 rounded-full bg-gradient-to-tr from-[#003B73] via-[#0B5198] to-[#0088FF] p-1 shadow-md mb-5">
                <div className="w-full h-full rounded-full bg-white flex items-center justify-center overflow-hidden">
                  {leader.photo ? (
                    ...
                    <span className="text-2xl font-black text-[#0B5198]">{leader.initials}</span>
                  )}
                </div>
              </div>

              <h3 className="text-lg font-bold text-[#0A2540]">{leader.name}</h3>
              <p className="text-sm font-semibold text-[#0B5198] mb-4">{leader.role}</p>

              <div className="relative">
                <Quote className="w-6 h-6 text-sky-200 absolute -top-3 -left-4" />
                <p className="text-sm text-slate-600 italic leading-relaxed px-2">
                  "{leader.quote}"
                </p>
              </div>
            </div>

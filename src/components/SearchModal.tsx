...
import { translatedTitle } from '../lib/courseTranslation';
...
const SECTION_IDS = ['home', 'solutions', 'why-us', 'training-modes', 'courses', 'leadership', 'mission-vision-values', 'testimonials', 'about'] as const;
...
export const SearchModal: React.FC<SearchModalProps> = ({ isOpen, onClose, onNavigate }) => {
  ...
  const tLearning = useTranslations('LearningOptions');
  const [query, setQuery] = useState('');
  const [activeIdx, setActiveIdx] = useState(0);
  const inputRef = useRef<HTMLInputElement>(null);
  const listRef = useRef<HTMLDivElement>(null);

  const KIND_META: Record<ResultKind, { badge: string; badgeClass: string }> = {
    Course: { badge: t('badgeCourse'), badgeClass: 'bg-sky-50 dark:bg-slate-800 text-[#0B5198] dark:text-sky-400' },
    Section: { badge: t('badgePage'), badgeClass: 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400' },
    Learning: { badge: t('badgeLearning'), badgeClass: 'bg-indigo-50 text-indigo-600' },
  };

  const index = useMemo<SearchItem[]>(() => {
    const courses: SearchItem[] = CATALOG.map((c) => {
      const title = translatedTitle(tCourses, c);
      ...
      return {
        ...
        href: `/courses/${c.slug}`,
        popular: c.popular,
        searchText: `${title} ${c.code ?? ''} ${c.level ?? ''} ${c.domain ?? ''} ${c.vendorName}`.toLowerCase(),
      };
    });

    const sections: SearchItem[] = SECTION_IDS.map((id) => {
      const label = t(`sections.${id}.label`);
      const sub = t(`sections.${id}.sub`);
      return { id: `sec-${id}`, kind: 'Section' as const, label, sub, target: id, searchText: `${label} ${sub}`.toLowerCase() };
    });

    const learningRaw = tLearning.raw('items') as string[];
    const learning: SearchItem[] = learningRaw.map((opt, i) => ({
      id: `learn-${i}`,
      kind: 'Learning' as const,
      label: opt,
      sub: t('learningOptionSub'),
      target: opt === LEARNING_OPTIONS[8] ? 'courses' : 'training-modes',
      searchText: opt.toLowerCase(),
    }));

    return [...courses, ...sections, ...learning];
  }, [tCourses, t, tLearning]);
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm nx-fade-in" onClick={onClose} />
      ...
      <div className="relative w-full max-w-xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden nx-pop-in">
        ...
        <div ref={listRef} className="max-h-[52vh] overflow-y-auto py-2">
          {flat.length === 0 ? (
            <div className="px-5 py-10 text-center">
              <p className="text-sm text-slate-500 dark:text-slate-400">{t('noMatches', { query })}</p>
              <p className="text-xs text-slate-400 mt-1">{t('tryHint')}</p>
            </div>
          ) : (
            grouped.map((group) => {
              const groupLabel = group.kind === 'Course' ? t('coursesGroup') : group.kind === 'Section' ? t('pagesGroup') : t('learningGroup');
                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={`shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full ${KIND_META[item.kind].badgeClass}`}>
        ...
        <div className="hidden sm:flex items-center gap-4 px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-900/60 text-[11px] text-slate-400">
          ...
          <span className="flex items-center gap-1.5">
            <kbd className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded px-1.5 py-0.5 font-sans">{t('escKey')}</kbd>
            {t('close')}
          </span>
          <span className="ml-auto">{t('resultsCount', { count: flat.length })}</span>
        </div>
      </div>
    </div>
  );
